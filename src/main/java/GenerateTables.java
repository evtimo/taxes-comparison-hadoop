...
public class GenerateTables {
    ...
    public static String QUERY = "CREATE TABLE IF NOT EXISTS employee ( eid int, " +
            ...
            "'Employee details' ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY  ' STORED AS TEXTFILE";
    ...
    public static String QUERY_CREATE_SELLER = " CREATE TABLE IF NOT EXISTS seller ( seller_inn varchar(10), seller_kpp varchar(9), " +
            ...
            "'Sellers transaction info' ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' LINES TERMINATED BY  ' STORED AS TEXTFILE";
    ...
    public static String QUERY_COMPARE_CUSTOMER_ERROR_HAS_NO_PAIR = "CREATE TABLE IF NOT EXISTS customer_errors_has_no_pair AS SELECT " +
            ...
            "c.total_with_tax_err AS total_with_tax_err, c.total_without_tax_err AS total_without_tax_err " +
            "FROM customer_errors c left outer JOIN " +
            "(SELECT " +
            ...
            "shp.total_without_tax_err AS total_without_tax_er " +
            "FROM customer_errors_customer_has_pair chp  full outer JOIN customer_errors_seller_has_pair sh " +
            ")" +
            "ON s.customer_inn_err_has_pair = c.customer_inn_err AND s.customer_kpp_err_has_pair = c.customer_kpp_err and " +
            "c.seller_inn_err_has_pair = s.seller_inn_err AND s.seller_kpp_err_has_pair = c.seller_kpp_err";
    ...
    public static String QUERY_COMPARE_SELLER_ERROR_HAS_NO_PAIR = "CREATE TABLE IF NOT EXISTS seller_errors_has_no_pair AS SELECT " +
            "s.customer_inn_err AS customer_inn_err, s.customer_kpp_err AS customer_kpp_err, s.seller_inn_err AS seller_inn_err, " +
            "s.seller_kpp_err AS seller_kpp_err, s.total_with_tax_err AS total_with_tax_err, s.total_without_tax_err AS total_without_tax_er " +
            ...
            "shp.total_with_tax_err AS total_with_tax_err, " +
            "shp.total_without_tax_err AS total_without_tax_er " +
            "FROM  seller_errors_customer_has_pair chp FULL JOIN seller_errors_seller_has_pair sh " +
            "ON chp.customer_inn_err = shp.customer_inn_err AND chp.customer_kpp_err = shp.customer_kpp_err AN " +
            "chp.total_with_tax_err = shp.total_with_tax_err AND chp.total_without_tax_err = shp.total_without_tax_er " +
            ")  " +
            "ON s.customer_inn_err = c.customer_inn_err AND s.customer_kpp_err = c.customer_kpp_err and " +
            "c.seller_inn_err = s.seller_inn_err AND s.seller_kpp_err = c.seller_kpp_er " +
